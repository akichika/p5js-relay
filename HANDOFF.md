...
現在バージョン: **v2.5.5**（v2.4.3までの経緯は本文中に残す。v2.5.0以降の
変更点は末尾の「8. v2.5.0以降の変更点」を参照）
...
6. **ストア審査対応**: ~~`host_permissions: ["<all_urls>"]`は送信先をユーザーが
   ...
   → **v2.6.0で対応済み**。実際にChrome Web Store提出時に
   「Broad Host Permissions」警告(詳細審査で公開遅延の可能性)を受けた
   ため、既知の8サイト(送信元5+デフォルト送信先3)のみ静的
   `host_permissions`、それ以外は`optional_host_permissions`から
   保存時に個別リクエストする方式に変更した。詳細は9章参照。
...
## 9. Claude/ChatGPT/GeminiのUI変更で壊れた時の対処方針(v2.5.5)

v2.5.1〜v2.5.4は、いずれも「AI側サイトのUI変更で抽出/転送が動かなくなった」
系の不具合対応だった。この種の問題は今後も起き続ける前提で、以下の
2段構えの備えを入れた。

### 9.1 バックアップ策(段階的フォールバック)

`extractCanvasCodeInPage()`の`collect()`は、上から順に以下を試す設計に
なっている。**新しい対応を追加する時は、既存の確実な方式より後ろ・
最終手段より前**に挿入すること:

1. `.cm-content`(CodeMirror6) / `window.monaco` / `.CodeMirror`(CM5) /
   `pre code, pre` — 汎用の既知エディタ実装
2. `.group\/line`(claude.ai専用の行ベース独自レンダラー、v2.5.3)
3. **最終手段(v2.5.5で追加)**: 上記が全て0件だった場合のみ、
   モノスペースフォントの葉要素(`getComputedStyle(el).fontFamily`に
   `mono`を含む)で最も長いテキストを候補にする。精度は低いが
   「何も転送されない」より「多少ずれたテキストでも転送される」方が
   ユーザーにとって次の一手(手動コピペ等)を考えやすい

トグル検出(`togglePat`)も同様に、既知の`role='radio']`等に加えて
`raw`などの一般的な語を緩く受け付けるようにしている。ただし
`view`/`show`のような汎用語は無関係な要素を誤クリックするリスクが
高いため**あえて含めていない**(一度試して却下した経緯あり)。
新しい語を追加する時は、幅120px以下という既存のサイズ制限
(`rect.width > 120`のスキップ)を必ず維持し、誤爆リスクを抑えること。

### 9.2 ユーザーへの報告導線

上記のフォールバックでも救えなかった場合、`errCanvasExtract`
(Canvasから抽出できません)と`errEditorNotFound`(エディタが
見つかりません)の2つのエラーメッセージに、GitHub Issuesへの
案内文を全10言語で追加した:
`github.com/akichika/p5js-relay/issues`

この文言は`_locales/*/messages.json`の該当キーに直接埋め込んでいる
(URLをクリック可能にするリッチテキストではなく、ボタンのツールチップ/
ポップアップのプレーンテキストとして表示されるため)。新しいエラー
メッセージを追加する場合、「AIサイトや送信先サイトのUI変更が原因で
起こりうる失敗」に該当するなら、同様にIssues案内を含めるかどうか検討
すること(逆に「送信先が未登録」等のユーザー設定起因のエラーには
不要)。

### 9.3 今後同種の不具合が来たら

1. まず`console.info("[p5.js Relay] extractFromTab debug:", ...)`
   (Service Workerコンソール、4.3節末尾を参照)で、どのフレーム・
   どの方式まで到達して失敗したかを確認する
2. 実際のサイトのDOMを直接調査する(可能なら`document.elementsFromPoint`
   や`getComputedStyle`で実要素を特定する。**素の`.click()`で反応しない
   要素に遭遇したら、内部にネストされた実`<button>`を探す**、というのが
   v2.5.4で得た教訓)
3. 新しいセレクタ/検出方式を追加する際は、既存の確実な方式を壊さないよう
   9.1の順序を守り、必ずコメントで「どのサイトの・いつの時点のUI変更への
   対応か」を記録すること(この積み重ねが本HANDOFF.mdの主な価値になっている)

## 10. host_permissions の設計(v2.6.0)

### 10.1 なぜ変更したか

v2.5.5までは`host_permissions: ["<all_urls>"]`だった。Chrome Web Store
への提出時に実際に以下の警告を受けた(ブロッカーではないが詳細審査で
公開が遅延する):

> Broad Host Permissions — Instead of requesting broad host permissions,
> consider using the activeTab permission, or specify the sites that
> your extension needs access to.
...
"host_permissions": [
  ...
  "https://editor.p5js.org/*", "https://openprocessing.org/*", "https://codepen.io/*"
],
"optional_host_permissions": ["<all_urls>"]
```

- **送信元5サイト+デフォルト送信先3サイト(計8)は静的`host_permissions`**。
  content_scriptsのmatchesと同じ5サイト、およびDEFAULT_RULESの3サイトと
  完全に一致させること。ここに無い/矛盾する状態になると、デフォルトの
  送信先がインストール直後から動かなくなる(下記10.3で検証済み)。
- **ユーザーが追加登録する任意の送信先は`optional_host_permissions`から
  実行時にリクエスト**。ceiling(申請できる上限)は`<all_urls>`だが、
  実際にリクエストするのは`chrome.permissions.request({origins:[...]})`
  に渡す**その1サイトのオリジンだけ**(`patternToOrigin()`で
  `urlPattern`からスキーム+ホストを抜き出し`/*`を付与)。この「上限は
  broadだが実リクエストはnarrow」という組み合わせが、Chromeの
  runtime host permissionsの想定パターンであり、`<all_urls>`を
  optional化しただけ(そのまま丸ごとリクエスト)では審査上の扱いが
  変わらない可能性が高いので注意。

### 10.3 権限リクエストのタイミング

`chrome.permissions.request()`は**ユーザー操作の文脈(トラステッドな
イベントハンドラ内)でなければ呼べない**。このため、送信(Transfer
ボタンクリック)時ではなく**オプションページの「保存」ボタンクリック時**
にまとめてリクエストする設計にした。送信時に権限が無いことが分かった
場合(`background.js`の`hasHostPermission()`)は、その場でリクエストせず
`errNoPermission`エラーを返し、オプションページで保存し直すよう案内する
(保存ボタンのクリックなら確実にユーザー操作の文脈になるため)。

**v2.6.1で踏んだ罠**: 「クリックのユーザージェスチャーは、クリック
ハンドラの中で複数回`await`しても普通は保持される」と思い込んでいたが、
実機では**`chrome.permissions.request()`の前に
`chrome.permissions.contains()`をループでawaitする事前フィルタを
入れていたところ、ジェスチャーが失効してダイアログが一切出なくなった**
(エラーも出ずに静かに失敗する)。対策として事前フィルタを廃止し、
クリックハンドラ内の**同期処理(入力検証)の直後、最初のawaitとして
`request()`を呼ぶ**ように変更した。既に許可済みのオリジンが混在して
渡されても、その分についてはダイアログが出ず即`true`になるだけなので、
「フィルタせず全部まとめて渡す」で実用上問題ない。
**今後`chrome.permissions.request()`が絡む処理を追加する時は、
クリックハンドラ内でそれより前に他の`await`を挟まないこと。**

`patternToOrigin()` は`options.js`と`background.js`の両方に同じ実装が
重複している(共有モジュールが無い構成のため)。片方だけ直すと不整合が
起きるので、変更時は両方揃えること。

### 10.4 今後の注意

- DEFAULT_RULES(p5js/openprocessing/codepen)のURLパターンを変更したり
  新しいデフォルト送信先を追加する場合、`manifest.json`の
  `host_permissions`にも対応するオリジンを追加すること。忘れると
  そのデフォルト送信先だけ`errNoPermission`で動かなくなる。
- 送信元サイトを追加する場合(新しいAIチャットサービス対応等)も同様に、
  `content_scripts.matches`と`host_permissions`の両方に追加が必要。
- まだ実ユーザーへの公開前(このHANDOFF更新時点)のため、既存ユーザーが
  既に`<all_urls>`を握っている状態からのマイグレーション処理は
  実装していない。将来的に非オフィシャルな配布等で先行ユーザーがいる
  場合は、`chrome.runtime.onInstalled`(reason: "update")で既存ルールの
  オリジンに対して権限が失われていないか確認し、オプションページに
  再許可を促す導線を追加すること。
